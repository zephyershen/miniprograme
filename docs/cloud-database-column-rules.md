...
会员客户端必须只调用 `columnHome`、`columnLesson`、`columnPractical`、`columnCases`、`columnCase` 和 `trendDossier`；其中除首页目录外，其余正文接口都会在服务端重新验证 Pro/管理员权益。内容管理接口只接受 `isActualAdmin` 为真的微信身份，管理员身份预览不能撤销这项真实权限，普通会员也不能通过伪造 action 取得草稿或上传图片。
