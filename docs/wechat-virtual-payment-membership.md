...
这里的“消息推送”是微信服务器向项目服务器发送的入站 Webhook，不是向用户微信发送通知，也不等于订阅消息。配置成功只证明微信完成了服务器地址验证，不能证明支付能拉起或实际 POST 事件已处理成功。

通知处理会校验 SHA-1 消息签名、AES-256-CBC 密文、PKCS#7 填充与 AppID；支付成功发货和退款通知还会绑定 OpenID、商户订单、商品、数量、环境和金额，并以官方查单为最终依据。微信要求重试时接口返回加密失败应答，不会在通知字段不一致时直接授予或回收会员。
