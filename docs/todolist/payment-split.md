...
4. `hyyc/cloudbaserc.json` 现在已经覆盖了绝大多数核心云函数，但 `deleteMessageCenterSession` 仍需手动补部署；环境变量仍然建议以云开发控制台为准核对。
...
- `hyyc/config/`
  - 前端访问控制配置
- `hyyc/pages/auth/_shared/community.js`
  - 注册页小区围栏配置
- `hyyc/utils/`
  - 公共工具
- `hyyc/components/`
...
- 注册页定位围栏现在只在“开发者工具本地模拟器”跳过；手机预览、体验版、正式版都会做真实定位校验。
...
- 当前依赖很轻：
  - `@babel/runtime`
- 全局 loading 和错误弹层已经去掉 `lottie-miniprogram`
- `project.config.json` 已对开发期文件做打包忽略，资源包也做过一轮 SVG / PNG 压缩以适配代码质量扫描
...
- 商品聊天和任务聊天已经接了全局消息中心与左滑隐藏，但还缺一轮真机回归验证排序 / 未读 / 跳转 / 新消息恢复
- 订阅消息已主动下线，当前只保留站内消息中心，不再做微信外部提醒
- 任务取消申请目前只有“同意”链路，没有“拒绝”入口；任务释放已支持“申请 -> 同意”，但也还没有“拒绝”
...
- `list_my_goods`
  - 返回：
    ...
    - 我买到的
  - 支持 `compact=true`
    - 这是“我的商品 / 我买到的”页当前使用的轻量模式，避免一次返回过大的商品对象
...
最近已经做过一轮性能和包体收敛，后续接手时不要无意打回旧实现。
...
- “我的任务”
  - 退款状态同步改成退避式单次调度
  - 前端优先走 `taskCancelFlow action=batch_sync_refund_status`
- “我的商品”
  - `getGoodsProfile action=list_my_goods` 当前走 `compact=true`
  - 页面数据做了轻量裁剪
- 消息中心
  - 只把当前展示所需数据放进页面 `data`
  - 支持左滑隐藏会话
...
### 14.3 `cloudbaserc.json` 已覆盖绝大多数核心部署清单

当前 `hyyc/cloudbaserc.json` 已经把绝大多数核心云函数都列进去了，包括：
...
- `deleteMessageCenterSession` 当前还没写进 `cloudbaserc.json`，部署时要手动补一次，或者后续把清单同步补进去
...
- 未接单任务可直接退款
- 已接单任务可申请取消并由对方同意
- 接单人接单后可申请“释放任务”，发布者同意后任务会重新开放接单
- 聊天里能发起查看手机号申请
- 对方同意后能看到完整手机号
...
- 左滑删除不生效
  - 先看 `deleteMessageCenterSession` 是否已部署
  - 再看 `message_center_hidden` 集合是否已自动创建
