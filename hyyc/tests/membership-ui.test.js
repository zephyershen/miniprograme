...
test('renders four native tabs and a real one-time Pro purchase entry', () => {
  ...
  assert.match(profile, /loading="\{\{billing\.purchasing \|\| accountAuthenticating\}\}"/);
  ...
  assert.match(profile, />退出登录<\/button>/);
  assert.match(profile, /已完成订阅账号验证/);
  assert.match(profile, /不会退出手机微信、取消会员或删除资料/);
  assert.doesNotMatch(profile, /当前未完成小程序登录/);
  assert.match(profileScript, /verifyMembershipAccount/);
  assert.match(profileScript, /membershipAccountVerified/);
  assert.match(profileScript, /forgetMembershipAccountVerification/);
  assert.match(profileScript, /不会退出手机微信、取消会员、删除头像昵称或清除未完成订单/);
  assert.match(profileScript, /promptProfileSetupAfterLogin/);
