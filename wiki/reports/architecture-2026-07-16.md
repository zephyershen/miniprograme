...
- WXML/WXSS 未改；6 个 `app.json` 正式页面入口均在 Node smoke test 中成功加载。首次真实微信编译发现 Node 与微信打包器的模块发现差异，随后已将运行时相对引用统一为带 `.js` 扩展名的具体模块直引并完成首页、详情复验。

## Unverified areas

- “运行时 import 已由页面入口 smoke test 覆盖”这一旧结论已被实际编译事故否定；当前已补做微信开发者工具普通编译、首页加载和详情打开，控制台无项目红色错误。
