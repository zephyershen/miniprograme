...
- 内容池中的资讯全部允许展示，但“108 条一次性下发和渲染”已在 2026-07-16 被服务端分页方案 `superseded`。首页首屏只取 8 条完整资讯，触底后每次追加 8 条；具有真实原文 Open Graph/Twitter 图片时优先显示原图，没有图片时使用受控原文页面截图，不生成或绘制无关替代封面，也不显示空白图片框。
- 外站图片只允许公网 HTTPS，限制重定向、超时、内容类型和 2.5MB 体积；通过服务端下载后写入当前 CloudBase 存储，前端不直接依赖任意第三方图片域名。
- 当时对 108 条候选完成一次整池封面检查，成功缓存 13 张真实原文封面；其余 95 条由独立 Playwright 服务生成 226 张原文页面截图。原图和截图维护都与资讯同步分离，避免外站超时拖慢首页；最终动态池状态见本文后续验证记录。
...
- 原文页面截图位于 `knowledge-previews/source/`，由维护令牌控制的 action 生成；最终动态池中 93 条缺封面资讯均有至少一张截图。
...
- `pages/feed-detail/index` 展示真实封面或原文截图、标题、来源、发布时间、30 秒导读、完整上游摘要分段、三条相关阅读及原始出处操作；截图可点击进入最多 3 张的 `wx.previewImage` 预览。
...
- 最新本地回归为 82/82 测试；项目检查覆盖 21 个 JSON、73 个 JavaScript 和 6 个页面。
