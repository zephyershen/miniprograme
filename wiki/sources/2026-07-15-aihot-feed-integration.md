...
- 详情页只保留原文链接，不显示聚合平台规范链接，也不复制原文正文；小程序提供显式复制原文操作。
...
- 新增 `pages/feed-detail/index`，展示封面、标题、中英文标题、摘要、来源、发布时间、热度及来源追踪操作。
...
- `npm test` 为 41/41；`npm run check` 检查 19 个 JSON、43 个 JavaScript 和 5 个页面通过。
