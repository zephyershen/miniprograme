...
- `knowledgeFeed` 公共 action 仅保留 `entitlements / feed / item / digest / digestReference`。
...
- `npm.cmd test`：168/168 通过。
- `npm.cmd run check`：27 个 JSON、141 个 JavaScript、9 个页面通过。
...
- 尚无第二个微信测试账号，当前管理员账号因管理员优先级不会显示 Pro 会员视图；人工会员授权应使用另一个账号。
