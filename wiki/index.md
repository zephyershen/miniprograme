...
- 后续只读复核确认体验版 `1bf9fb8`、CI/业务代码锚点 `e2ef825` 与线上四函数的业务
  运行代码同源，但 `develop`/`trial` 有意禁止生产写操作。截图执行层仍有四处
...
- 新专栏函数已部署，微信开发者工具真实页面自动化已读取合同版本 4、24 节基础课、6 节动手课、会员无锁状态、平台安装详情、无图资讯正文和无个人影响模块的简报；首发前先关闭 P2 日志残余并完成凭据轮换、普通/Pro 双身份物理真机验收、名称备案、类目隐私和微信审核发布。真实订阅充值/退款仍是开售前门禁，支付保持关闭时不阻塞免费资讯首发。其他频道、多来源和长期资源观察均为上线后扩展或运维任务。
