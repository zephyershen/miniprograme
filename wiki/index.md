...
- 因 `d88c155` 又修改了 `knowledgeFeed`，正式微信上传前须从该代码锚点执行一次
  code-only 部署、manifest 回读及普通/Pro/真实管理员 canary；本轮未部署生产。
- 最终干净工作树验证为 38 个 JSON、355 个 JavaScript、16 个页面、777/777 个
  Node 测试、29 个集合与 50 个索引；覆盖率和生产依赖审计通过。微信开发者工具从
  精确代码提交生成 632,253-byte 预览包。
- 小程序没有上传新体验版、提交审核或发布。历史记录为 `2.3.4` 曾进入审核、公开线上
  为 `2.3.2`，正式动作前必须先在公众平台确认当前状态和可用版本号。
...
- 推荐的最终顺序为：推送分支与新 RC 标签 → code-only 部署 `knowledgeFeed` 并
  canary → 确认平台版本状态 → 可选真机扫码 → 上传候选 → 提交审核。
