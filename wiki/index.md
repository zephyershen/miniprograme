...
- 近 7 天 AI/科技精选已在后台接入；首页首屏只取 8 条，触底后每次再取 8 条。资讯源正常缓存 TTL 为 15 分钟，用户下拉刷新在缓存满 1 分钟后可以强制同步；新资讯先进入内部缓存，视觉维护任务每 5 分钟准备真实封面或原文截图，只有视觉就绪后才公开。2026-07-16 最终动态池为 106 条且全部有视觉素材：13 条真实封面、93 条受控原文页面截图；公开首页不显示聚合平台、接口或缓存信息。娱乐、社会、游戏和英语仍待独立来源。
- 首页支持时间、公司与模型、技术方向组合筛选；筛选项附结果数，空选项不可选。“最新”对全部资讯按发布时间倒序，“热度”对全部资讯按热度倒序，不再单独处理放大主稿。
- 资讯详情提供原文截图多图预览、30 秒导读、完整上游摘要分段和三条相关阅读，不再按字符截断或添加人工省略号；“来源与原文”位于“接着看”之前，URL 可展开/收起并可复制。当前没有已验证的外部业务域名，微信也不能直接唤起任意系统浏览器，因此外部原文点击会复制链接并提示到手机浏览器粘贴。
- 原文截图服务运行在独立公网服务器并通过回环 Mihomo 代理访问外站；CloudBase 负责定时编排、上传实际 JPEG、保存文件 ID 和公开数据。服务、代理、Nginx 与定时触发器当前正常，视觉回填失败 0、待发布 0、待清理 0。迁移部署时必须复制同等私网出口阻断。
