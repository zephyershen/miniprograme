...
这是一个从个人文章消化箱迁移为编辑型知识获取平台的微信小程序：当前后台保留近 7 天 AI/科技资讯池，首页以非卡片式结构首屏加载 8 条、触底再追加 8 条，并可按时间、公司与模型、技术方向筛选；默认把当前范围内热度最高的一条作为放大主稿，其余资讯按发布时间从新到旧，用户也可切换为整列热度排序。有图时显示真实原图，无图时使用纯文字编辑行。详情用可选图片、30 秒导读、完整上游摘要分段和相关阅读降低阅读负担。个人导入/待处理流程已从首页撤下，娱乐、社会、游戏和英语仍待各自内容源。
...
| 6 个页面、3 个云函数及本地测试已实现 | confirmed | 当前代码、67 个 Node 测试 |
...
- 首页主稿与排序：默认选中“最新”；服务端先在当前频道和筛选范围内选出热度最高的主稿，再将其从普通列表移除，其余资讯显式按发布时间从新到旧。用户切换“热度”后整列按上游热度值从高到低，同热度按发布时间倒序。两种模式都在分页前执行且不会重复主稿，切换会从第一页重新加载。
...
- `git diff --check`：通过。
- 微信开发者工具已重新编译当前项目并实际打开资讯首页与资讯详情；模块加载红色错误已清空，2026-07-16 最新首页返回 107 条资讯，仅剩基础库和 SharedArrayBuffer 的开发者工具提示。
...
- 三个 Node.js 18.15 云函数已部署；`knowledgeFeed` 当前使用服务端排序与分页，并已部署模块化版本。2026-07-16 最新线上回归：默认查询返回 `sort: latest` 和 107 条结果；首条放大主稿热度为 86，其后发布时间依次为当日 11:27、10:00、09:36、08:33、08:12，证明主稿热度优先且普通列表时间倒序。有效详情返回 3 条相关阅读，无效 ID 返回稳定 `ITEM_NOT_FOUND`。无图条目和无图相关阅读仍能打开详情；公开响应已移除聚合平台页面、归因和平台标识字段。
