...
2026-07-28 功能基线 `68382f5` 已把 24 节基础课和 6 节动手课改为可持续更新的
...
同构归一化。评论隐藏实现锚点 `5acb52e` 已 code-only 部署并完成 118 个受控文件
线上哈希回读；安全打包锚点 `fffd078` 的微信开发版本 `2.4.2` 已上传。正式线上最后确认为 `2.3.5`，尚待
用户本人确认平台审核规则并提交。

2026-07-28 已上传的 `2.4.2` 替换候选把评论设为确定性的客户端关闭态：资讯列表和详情不
显示“评论”、评论数量、气泡或弹层，旧评论深链不能重新打开；会员权益、消息中心
和个人资料也不再显示评论文案或评论通知，通用消息入口改为信封。评论后端、审核、
治理与历史数据仍保留，未来恢复需修改中央发布开关并提交新的微信审核版本，不能在
审核通过后远程热开启。完整回归为 792/792，开发者工具六页实画面 canary 已通过。
新版消息请求还会让 `knowledgeFeed` 越过评论通知继续寻找会员/资料消息，并拒绝关闭
态“全部已读”；该 code-only 更新已完成 118 文件线上哈希回读，不改集合、索引、
生产数据或存储规则。
...
已验证版本，`knowledgeFeed` 已从评论隐藏锚点 `5acb52e` code-only 更新并回读收敛。
搜索存量 8,152 条已全部扫描，
`ready=true`、待失败 0、隔离 0。真实管理员三角色、双图草稿/预览/发布/下架、
普通标题目录、Pro 正文与进度均通过生产 canary；完整标题全角标点搜索也在最终
canary 中命中。安全微信开发版本 `2.4.2` 已上传；正式线上最后确认为 `2.3.5`，尚待
用户本人确认审核规则并提交，未正式发布。
...
| 16 个注册页面、4 个正式云函数及本地测试已实现 | confirmed | 最终干净工作树、792/792 个 Node 测试；含搜索页、两个真实管理员专栏页面、一个兼容趋势路由和小程序打包边界守卫 |
...
| 资讯互动与会员转化界面 | uploaded-candidate-comments-hidden | `2.4.2` 仅显示喜欢、收藏、分享，列表和详情均无评论气泡或空槽；“我的”页和受限入口共用六项可见 Pro 权益、服务端价格与动态折扣，评论后端保留但不可达 |
| 公开资料异步审核与隐私文案 | confirmed | 私有 staging/review、独立审核队列、租约重试、所有者候选读取和“审核中”界面已部署；旧已通过资料在新候选通过前继续公开，真实账号 canary 待完成 |
| 评论异步审核与站内消息 | backend-retained-ui-paused | 评论私有 pending、限流、后台租约审核、事务公开、outbox、两级线程和治理后端仍保留；已上传 `2.4.2` 过滤评论通知、跨页保留会员/资料消息并重算可见未读，不显示评论分类、入口、弹层或文案，未来恢复需重新送审 |
...
| 微信版本上传与审核 | uploaded-awaiting-user-review-submit | 安全候选 `2.4.2@fffd078` 已从零 `node_modules` 的独立干净工作树上传，preview 637,272 bytes、upload 639,674 bytes；误带服务端源码的 `2.4.1` 不得提交；尚待用户本人确认审核规则并提交，未代签、未正式发布 |
| 最终代码与生产后端一致性 | confirmed | 评论实现锚点 `5acb52e` 已 code-only 部署 `knowledgeFeed`；最终打包锚点 `fffd078` 对云函数零改动；四函数 manifest、10 个触发器、118 个受控源码 SHA-256、打包边界守卫与评论隐藏六页 canary 均通过；RC 标签为 `rc/2026-07-28-comments-hidden-v2` |
...
- `npm.cmd run verify`：通过项目结构、环境、数据库、索引、全量 Node 测试、
  覆盖率和生产依赖审计。
- 隔离干净工作树的 `npm.cmd run check`：38 个 JSON、358 个受控 JavaScript、
  16 个注册页面通过结构、语法与微信模块引用兼容性检查；客户端检查包约 0.73 MiB。
- `npm.cmd test`：792/792 个 Node 测试通过；完整 `npm.cmd run verify` 同时通过
  环境、29 个集合、50 个索引、覆盖率和生产依赖审计。
- `git diff --check`：通过。
- 微信开发者工具 CLI `auto`、生产页面自动化与 `preview`：通过；锁定代码提交
  `fffd078` 的预览包为 637,272 bytes，安全 `2.4.2` upload 回执为 639,674 bytes。资讯时间线续载实测外层命中区
  47px、内部按钮 35px，错误轨道可见且空日期没有重复 loading；搜索结果跳转周简报
  后正确激活 `7d`。微信开发版本 `2.4.2` 已上传；尚待用户本人确认审核规则并提交。
...
1. 已上传安全评论隐藏开发版本 `2.4.2` 并替换旧 `2.4.0` 与不安全的 `2.4.1`；
   只提交 `2.4.2`，由用户本人确认平台审核
   规则并提交审核，审核通过后再显式发布。自动化不代签该确认。
...
7. 专栏、学习进度、搜索、8,152 条回填、真实管理员双图发布和最终三角色 canary
   已完成；评论隐藏兼容更新也已从 `5acb52e` code-only 部署，四函数 manifest、
   10 个触发器和 118 个受控文件哈希回读一致。分支和 RC 标签
   `rc/2026-07-28-comments-hidden-v2` 已推送，安全微信开发版本 `2.4.2` 已上传。
