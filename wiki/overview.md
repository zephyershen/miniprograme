...
这是一个从个人文章消化箱迁移为编辑型知识获取平台的微信小程序：当前首页以非卡片式结构展示 AI/科技图文资讯，可按时间、公司与模型、技术方向筛选；详情用真实图片、30 秒导读、完整上游摘要分段和有图相关阅读降低阅读负担。个人导入/待处理流程已从首页撤下，娱乐、社会、游戏和英语仍待各自内容源。
...
| 6 个页面、3 个云函数及本地测试已实现 | confirmed | 当前代码、51 个 Node 测试 |
...
| AI/科技精选聚合、筛选、缓存和原始来源追踪 | confirmed | 后台近 7 天 108 条云端实测、首页 13 张原文封面、模拟器渲染 |
| 完整摘要分段和有图相关阅读 | confirmed | 云端 361 字摘要无人工省略、3 条有图相关阅读 |
...
- 首页筛选：24 小时、近 3 天、近 7 天；15 个公司与模型主题；14 个技术方向。三个维度可组合，选项显示当前图文数，零结果项不可选。
...
- 公共资讯缓存 15 分钟并使用 ETag；当前近 7 天缓存 108 条。原文封面经安全校验后写入 `knowledge-covers/aihot/`，当前 13 个可用对象；首页只显示这 13 条有真实图片的资讯，不生成几何替代图。
...
- `git diff --check`：通过。
- 微信开发者工具已编译并成功渲染收敛后的纯资讯首页：精选 13、AI 前沿 8、科技 5，并显示本周收录 108。
...
- 三个 Node.js 18.15 云函数已部署；`knowledgeFeed` 已通过 108 条真实聚合数据、分页缓存、13 张真实封面和详情读取验证。单条详情在线完整返回 361 字摘要和 3 条有真实图片的相关阅读，公开响应已移除聚合平台页面、归因和平台标识字段。
