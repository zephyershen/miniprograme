...
| 12 个注册页面、4 个正式云函数及本地测试已实现 | confirmed | 当前代码、621/621 个 Node 测试与四函数白名单回读通过 |
...
- `npm test`：621/621 通过。
- `npm run check`：32 个 JSON、301 个 JavaScript、12 个注册页面通过结构、
...
4. 用真实新账号和 iPhone 真机确认第一次点击只登录、不建单、不拉起收银台，
   可跳过头像昵称，第二次点击才进入支付；再覆盖成功、取消、异常恢复、重复查单、
