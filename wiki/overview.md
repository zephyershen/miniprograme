...
业务提交 `1921a9d` 已推送并恢复生产函数可追溯性；微信 `2.3.3` 从该提交的干净
工作树上传，并于 2026-07-25 18:18:02 进入审核。公开线上仍为 `2.3.2`。剩余边界
是审核通过后的正式发布、真实手机评论 canary，以及 2026-08-06 前的依赖复审。
...
| 微信版本上传与审核 | confirmed | `2.3.3` 审核包 1,418,704 bytes，绑定 `git-1921a9d`；2026-07-25 18:18:02 提交后状态为“审核中”，公开线上仍为 `2.3.2` |
| 业务代码锚点与线上运行代码同源 | confirmed | 业务提交 `1921a9d` 已推送至当前远端分支；评论线程、消息删除、图片自动处理与审核刷新均从该提交进入 `2.3.3` 审核包 |
...
- 当前 26 个合同集合均由云函数管理并为 `ADMINONLY`。互动、评论、公开资料、资料审核队列、消息 outbox、收件箱与会员 checkout 租约分别按 owner、item、状态和到期时间使用合同索引。
- 四个正式函数保留：三个 Node.js 18.15 函数和 Node.js 20.19 的
  `sourcePreviewWorker`。生产 `knowledgeFeed` 已从代码锚点 `9da8cbd` 部署
  24 节基础课、6 节动手课、
...
1. 等待微信 `2.3.3` 审核完成；通过后执行正式发布。审核期间公开线上仍为
   `2.3.2`，不要重复提交同一开发版本。
