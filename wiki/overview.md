...
2026-07-28 生产后端锚点 `03657d7` 已把 24 节基础课和 6 节动手课改为可持续更新的
...
2026-07-28 学习进度和全站搜索的生产合同已完成增量发布；`knowledgeFeed` 与
`knowledgeOps` 从提交 `03657d7` 部署并回读收敛。搜索存量 8,152 条已全部扫描，
`ready=true`、待失败 0、隔离 0。真实管理员三角色、双图草稿/预览/发布/下架、
普通标题目录、Pro 正文与进度均通过生产 canary。最终代码 `d88c155` 又修改了
`knowledgeFeed` 的日期窗口、免费搜索和进度 revision 逻辑，因此微信上传前仍须从
该代码锚点做一次 `knowledgeFeed` code-only 部署、回读与三角色 canary；本轮按要求
没有执行该生产动作。
...
| 16 个注册页面、4 个正式云函数及本地测试已实现 | confirmed | 当前工作树、777/777 个 Node 测试；含搜索页、两个真实管理员专栏页面和一个兼容趋势路由 |
...
| 微信版本上传与审核 | needs-current-status-check | 历史记录为 `2.3.4` 于 2026-07-25 19:47:21 进入审核、公开线上为 `2.3.2`；该平台状态本轮未重新确认。最终候选 632,253 bytes 已预览但未上传或提交审核 |
| 最终代码与生产后端一致性 | needs-code-deploy | 生产后端来自 `03657d7`；最终小程序代码锚点为其后代 `d88c155`，其中包含 `knowledgeFeed` 修复。微信上传前须 code-only 部署并 canary。旧 RC 标签保持不动，最终 RC 使用新标签 `rc/2026-07-28-member-admin-global-search-v2` |
...
- `npm.cmd run verify`：通过项目结构、环境、数据库、索引、全量 Node 测试、
  覆盖率和生产依赖审计。
- `npm.cmd run check`：38 个 JSON、355 个 JavaScript、16 个注册页面通过结构、
  语法与微信模块引用兼容性检查；客户端检查包约 0.72 MiB。
- `npm.cmd test`：777/777 个 Node 测试通过；完整 `npm.cmd run verify` 同时通过
  环境、29 个集合、50 个索引、覆盖率和生产依赖审计。
- `git diff --check`：通过。
- 微信开发者工具 CLI `auto`、生产页面自动化与 `preview`：通过；锁定代码提交
  `d88c155` 的预览包为 632,253 bytes。资讯时间线续载实测外框 380×47px、
  按钮 148×35px、失败按钮
  119×35px，失败轨道可见且空日期没有重复 loading。本轮未执行正式上传或审核提交。
...
- 四个正式函数保留：三个 Node.js 18.15 函数和 Node.js 20.19 的
  `sourcePreviewWorker`。生产 `knowledgeFeed` 与 `knowledgeOps` 已于 2026-07-28
  从提交 `03657d7` 部署并回读收敛；前者继续包含
  24 节基础课、6 节动手课、
...
1. 先在微信公众平台确认历史 `2.3.4` 的当前审核和线上状态；若仍在审核，不要
   重复提交同一开发版本。若已通过、发布、撤回或驳回，再据此选定新候选版本号。
...
7. 专栏、学习进度、搜索、8,152 条回填和真实管理员双图发布 canary 已完成。
   上传前先从 `d88c155` code-only 部署 `knowledgeFeed`，回读 manifest 并复核普通、
   Pro、真实管理员的滚动日期、标题级专栏搜索和进度写入；再确认公众平台历史
   `2.3.4` 状态并选定新版本号。建议先把当前分支和新 RC 标签
   `rc/2026-07-28-member-admin-global-search-v2` 推送备份，再使用已生成的预览
   二维码做一次可选物理手机冒烟。
