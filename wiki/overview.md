...
这是一个从个人文章消化箱迁移为编辑型知识获取平台的微信小程序：当前首页以非卡片式结构展示 AI/科技图文资讯，详情用真实图片、30 秒导读、三条要点和有图相关阅读降低阅读负担；个人导入/待处理流程已从首页撤下，娱乐、社会、游戏和英语仍待各自内容源。
...
| 6 个页面、3 个云函数及本地测试已实现 | confirmed | 当前代码、45 个 Node 测试 |
...
| 短读资讯详情和有图相关阅读 | confirmed | 云端单条详情返回 3 条有图相关阅读、微信开发者工具渲染 |
...
- 本轮详情改版前的完整仓库快照：`da5c4bb2a50b4524912dd9c228400fc0ea3af396`
...
- 详情页不复制原文正文：导读和三条要点只基于上游摘要裁剪，相关阅读只选真实有图条目。
...
- `npm test`：45/45 通过。
- `npm run check`：20 个 JSON、46 个 JavaScript、6 个页面通过结构与语法检查。
- `git diff --check`：通过。
...
- 三个 Node.js 18.15 云函数已部署；`knowledgeFeed` 已通过真实聚合数据、缓存和详情读取验证，单条详情在线返回 3 条有真实图片的相关阅读，公开响应已移除聚合平台页面、归因和平台标识字段。
