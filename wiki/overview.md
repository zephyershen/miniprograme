...
2026-07-23 的 [全项目生产就绪审计](reports/production-readiness-audit-2026-07-23.md) 结论为 **Fail**：450/450 测试、项目检查和微信 preview 编译均通过，但生产媒体所有权/规则、会员降级详情缓存、截图 DNS rebinding、依赖健康与发布快照仍有阻断项。此前各功能局部“已验证”不应被解读为整个当前工作树已可安全发布。
...
| 11 个注册页面、6 个云函数及本地测试已实现 | confirmed | 当前代码、450/450 个 Node 测试通过 |
...
| GitHub 开源库来源范围 | confirmed | 服务端同步版本 3 已部署；生产 1,727 条 `openSource` 记录、528 个原生标签，分钟指纹轮询、全量历史和官方头像均已验证 |
| 来源头像客户端可读链路 | confirmed | 真实来源头像优先；缺失或续签失败时使用本地产品图标，批量换签、列表/详情与开发者工具预览均已验证；修复仍待随完整客户端上传体验版 |
...
| 公开资料审核与隐私文案 | needs-review | 审核本身 fail-closed，但生产媒体路径/规则与删除操作缺少对象级所有权，详见 2026-07-23 生产审计 |
...
| 微信虚拟支付会员闭环代码 | confirmed | `pro_30d` 方案 `available=true`；官方查单/发货、对账、iOS 退款询问、安全通知与幂等回收已部署，待真机支付验收 |
...
- 资讯顶部：来源范围为“全部 / 官方动态 / 资讯 / 推文 / GitHub”。`官方动态` 只是 `firstParty` 的用户界面名称，内部来源合同不变；前四个使用 AIHOT 上游成员关系而不是本地关键词分类。`GitHub` 使用 AIGCLINK 公开方案索引并由独立同步服务维护，卡片来源显示为“GitHub 开源库”；“精选”作为独立会员能力入口，不参与来源过滤。独立频道使用连续卡片列表，不显示日期分组、折叠、时间线或时间点，只按 AIGCLINK 原生标签筛选且不限时间；同一条目在“全部”频道仍按普通资讯时间线、主题和权限显示。生产已同步 1,727 条记录和 528 个标签。
...
- 云函数：`digestIngest`、`digestStore`、`knowledgeFeed`、截图执行函数 `sourcePreviewWorker`、私有运维入口 `knowledgeOps`、微信虚拟支付入口 `membershipBilling`
...
- `npm test`：450/450 通过。
- `npm run check`：35 个 JSON、271 个 JavaScript、11 个注册页面通过结构、语法与微信模块引用兼容性检查。
- `git diff --check`：通过。
...
- 六个 Node.js 18.15 云函数保留；生产 `knowledgeFeed` 已部署 24 节基础课、6 节动手课、合同版本 4、24 节基础课各三页受保护手绘图、四分钟有界等图与失败/超时文字放行、公平视觉队列、AIHOT 作者/头像/原始标签增强、CloudBase 主模型、Packy 兜底、60,000 点月预算熔断、评论/资料审核与历史补审。部署后真实 feed/item 调用成功且无新增索引错误。`knowledgeOps` 无触发器且受维护令牌保护。`membershipBilling` 已实现微信虚拟支付事件函数、每 15 分钟的有界最旧优先对账和安全模式退款通知；订单/会员 ACL 为 `ADMINONLY`。现网 `plans.available=true`，通知 Token/AESKey 未配置时 HTTP 入口独立安全关闭。
...
1. 在体验环境补齐普通/Pro 双身份、24 节基础课与 6 节动手课逐项打开、无图列表/详情和手绘图加载验收；历史周案例/趋势集合继续保持客户端不可读写，但不再作为首发产品路径。
2. 完成一次真实订阅充值联调：在微信后台和 CloudBase 成对配置消息推送 Token/EncodingAESKey，随后覆盖 Android/iOS 支付成功、取消、异常退出恢复、重复查单、发货、iOS 退款询问、退款通知、退款/到期后重新锁定及普通/Pro 账号付费内容访问。`pro_30d`、590/1090 分价格、正式支付密钥和 IAP 已配置或确认；当前生产订单数为 0，所以仍不能把自动化测试当作真实扣款验收。一次购买 30 天，不是自动续费。
3. 完成备案与微信发布流程：等待名称审核，按实际“资讯阅读 + 会员评论 + 虚拟内容付费”变更备案和服务类目/隐私说明，上传体验版完成头像、相册原图、键盘、分享和会员路径真机验收，再提交微信审核发布。扫码、验证码和最终提交仍由管理员完成。
