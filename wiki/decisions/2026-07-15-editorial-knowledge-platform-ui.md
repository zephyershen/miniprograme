...
- 所有已缓存资讯都进入视觉准备流程：真实原文图片优先；没有封面时使用受控原文页面截图。只有视觉准备完成的条目才进入公开列表、详情、筛选计数和相关阅读；不得生成与原文无关的分类几何封面或 AI 配图，也不保留空白图片占位。
- 频道导航下提供非卡片式筛选入口，支持 24 小时、近 3 天、近 7 天，以及公司与模型、技术方向的组合筛选；每个选项显示当前资讯数量，没有结果的选项弱化且不可选。
- 默认“近 7 天”代表完整上游精选池；视觉尚未准备完成的条目保留在内部缓存中等待发布，而不是从内容池永久删除。
...
- “无图资讯固定使用纯文字编辑行”已于 2026-07-16 被 [缺图资讯使用受控原文页面截图](2026-07-16-source-preview-renderer.md) `superseded`；截图临时失败时条目保持待发布状态，30 分钟后重试，不先以纯文字形式暴露给用户。
