...
- `knowledge_memberships` 与 `knowledge_membership_orders` 为 `ADMINONLY`。会员
  ...
  支付结果也不能直接授予权益；只有微信虚拟支付服务端查单确认订单号、环境、类型、
  标价和实付金额全部一致后，事务才发放一次性 30 天 Pro。
...
- 当前本地回归为 621/621 个 Node 测试；项目检查覆盖 32 个 JSON、301 个
...
关闭；`qwen3.5-flash` 与 `qwen3.5-plus` 已启用。当前客户端与支付代码锚点为
`d927bfb`，`knowledgeFeed` 生产业务锚点仍为 `2db8dce`；`membershipBilling`
已从新锚点部署。微信版本 `2.3.1` 已上传并处于审核流程；另生成了
462,604 bytes 的最新开发预览包，尚未替换已上传版本。
