...
- 喜爱与收藏对所有真实微信用户开放。评论后端只允许 Pro/管理员并由服务端重鉴权，
  已上传的安全 `2.4.2` 替换候选通过 `PRODUCT_FEATURES.comments=false` 暂时隐藏全部评论入口、
...
- `knowledge_memberships` 与 `knowledge_membership_orders` 为 `ADMINONLY`。会员
  订阅第一次点击只执行 `wx.login` 与服务端 `code2Session` 身份一致性校验，
  不创建订单、不拉起收银台；第二次点击才重新取得登录码、建单并支付。登录步骤
  缓存按匿名查看者分区隔离，付款时仍重做权威校验。`wx.login` 不弹出账号选择或
  资料授权页，头像昵称只能由用户通过官方控件主动选择，且不是支付硬门槛。客户端
  的“退出登录”只删除当前查看者分区的本机订阅验证标记，不退出手机微信、不切换
  微信账号，也不取消会员、删除资料或清除待支付订单。支付结果不能直接授予权益；
  只有微信虚拟支付服务端查单确认订单号、环境、类型、标价和实付金额全部一致后，
  事务才发放一次性 30 天 Pro。
...
- 当前本地回归为 792/792 个 Node 测试；隔离干净工作树项目检查覆盖 38 个 JSON、
  358 个受控 JavaScript 和 16 个注册页面。GitHub 全量库、全页懒加载转圈、稳定互动、
  权益缓存、用户媒体、截图网络边界、模型主备路由、预算熔断、评论与资料后台
  审核、站内消息、微信登录绑定、订阅验证退出、虚拟支付签名/查单及生产自动队列
  均已验证；最终代码的专栏管理、学习进度、全站搜索、滚动日期窗口、Tab 跳转和
  管理排序边界也已纳入回归。
...
关闭；`qwen3.5-flash` 与 `qwen3.5-plus` 已启用。评论隐藏实现锚点为
`5acb52e76da8c8a7feb6bdbc082415eb73fda39a`，最终安全打包锚点为
`fffd078c4e30f3d7f03f3e731b1183ebebb6aeb6`。生产 `knowledgeFeed` 已从前者
code-only 更新，后者对云函数零改动；四函数 manifest、10 个触发器、线上下载的
118 个受控文件哈希和评论隐藏六页 canary 均通过，`knowledgeOps` 保持已验证版本。
安全微信开发版本 `2.4.2` 已从零 `node_modules` 的独立干净工作树上传，预览为
637,272 bytes、upload 回执为 639,674 bytes；误带服务端源码的 `2.4.1` 不得提交。
正式线上最后确认为 `2.3.5`。尚待用户本人确认审核规则并提交，未正式发布。
...
允许后才事务公开；审核结果继续保留在消息数据中。2026-07-28 已上传的安全 `2.4.2` 暂时
