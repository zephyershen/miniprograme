...
- `knowledge_memberships` 与 `knowledge_membership_orders` 为 `ADMINONLY`。购买前先
  显式确认当前微信账号、金额和期限，再静默执行 `wx.login`；服务端以
  `code2Session` 确认 OpenID 与当前云函数 actor 一致。`wx.login` 不弹出独立
  登录页，客户端支付结果也不能直接授予权益。只有微信虚拟支付服务端查单确认
  订单号、环境、类型、标价和实付金额全部一致后，事务才发放一次性 30 天 Pro。
...
- 当前本地回归为 620/620 个 Node 测试；项目检查覆盖 32 个 JSON、300 个
...
`d2434be`，`knowledgeFeed` 生产业务锚点仍为 `2db8dce`；`membershipBilling`
已从新锚点部署。微信版本 `2.3.1` 已上传并处于审核流程；另生成了
459,512 bytes 的最新开发预览包，尚未替换已上传版本。
