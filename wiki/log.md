...
## [2026-07-23] paid-membership-release-and-review | 开启会员支付并提交 2.3.1 审核

- Session: local Codex task
- Scope: 支付发布开关、微信虚拟商品与消息回调、生产索引/函数部署、微信版本上传、
  服务类目、隐私保护指引和审核提交；未执行真实资金扣款或退款。
- Code: 业务提交 `82e88f4` 已推送，551/551 测试和 GitHub Actions
  `production-verify` 通过；35 个合同索引、22 个集合/规则和四函数回读收敛。
- Payment: `pro_30d` 微信商品与服务端统一为一次购买 30 天、5.9 元，
  `plans.available=true`；消息推送为安全模式 JSON，加密回调探针通过。
- Governance: 评论删除、三人举报隐藏、申诉/恢复、作者精确私有查询和媒体耐久清理
  已部署；过期会员仍可管理自己的历史私有治理记录。
- WeChat: 线上版本为 2.3.0；递增版本 2.3.1（406,258 bytes）已上传。
  “资讯 > 信息资讯”服务类目已通过并设主营；隐私指引移除旧手机号/位置，登记
  图片、复制、昵称头像、会员订单和评论。审核于 22:48:33 进入“审核中”。
- Remaining: 等待微信审核和发布；用真实手机补齐扣款、取消、恢复、退款及
...
## [2026-07-24] scoped-membership-logout | 增加订阅验证退出登录

- Session: local Codex task
- Scope: 仅修改小程序端订阅验证状态、我的页退出交互、并发保护、测试、开发预览
  和项目 Wiki；未修改云函数、商品价格、会员权益、生产数据或微信已上传版本。
- Semantics: “退出登录”只删除当前查看者分区的本机
  `membership_account_verification_v1`，不会退出手机微信、切换微信账号、取消
  会员、删除头像昵称或清除待支付订单。用户如需切换微信账号，必须在微信侧操作。
- Reliability: 退出前二次确认并锁定重复点击；登录/退出完成时会校验页面可见性和
  操作代次，隐藏或卸载后不再弹窗、跳转或 `setData`。在途会员与价格请求正常
  收敛，并在提交前重新读取本机验证状态。
- Verification: 客户端提交 `f522b64`；退出专项 37/37、全量 Node 626/626，
  32 JSON/301 JavaScript/12 pages、70 项运行环境合同、25 个集合规则、
  43 个索引、覆盖率和生产依赖审计全部通过。
- Preview: 微信开发者工具编译成功，最新开发预览包 467,606 bytes。由于本次为
  client-only 修改，未重部署四个正式云函数。
- Memory: 更新两步登录 source、异步审核 decision、导航、总览、小程序实体和
...
## [2026-07-25] comment-interactions-review-2.3.3 | 提交评论与消息修复版本审核

- Session: local Codex task
- Scope: 对评论回复层级、回复计数、主评论展开收起、大图自动处理、微信头像续签、
  资讯下拉刷新、审核结果自动刷新和消息左滑删除做最终发布检查；没有修改生产数据、
  云存储对象、会员订单或资金状态。
- Verification: 完整 `npm.cmd run verify` 通过，Node 708/708；项目检查为
  33 JSON、316 JavaScript、13 pages 和 0.60 MiB 客户端源码。26 个合同集合、
  44 个合同索引、存储规则、四个正式函数及触发器 manifest 均在线回读收敛，
  `git diff --check` 通过。
- Code: 业务提交 `1921a9d` 已推送至
  `origin/codex/rebuild-digest-inbox`。一处独立的收藏卡片样式工作树改动没有进入
  提交；微信包从该提交的隔离干净工作树生成。
- WeChat: `2.3.3` 上传包为 1,418,704 bytes，项目备注绑定
  `git-1921a9d`。送审表单更新为本次评论、图片、审核刷新和消息删除测试路径；
  沿用上一审核通过版本的隐私与运行配置。
- Review: 微信公众平台于 2026-07-25 18:18:02 回读 `2.3.3 / 审核中`，
  平台预计 1–7 天且当前没有加急额度。公开线上版本仍为 `2.3.2`，审核通过后
  还需单独执行正式发布。
- Sensitive handling: Wiki 未记录公众平台会话参数、管理员身份标识、OpenID、
  Cookie、访问令牌、用户数据、订单号、评论正文、图片或原始浏览器日志。

## [2026-07-25] membership-renewal-review-2.3.4 | 会员续费文案与咨询方式重新送审

- Session: local Codex task
- Scope: 会员身份的购买按钮精确显示“续费”；会员页底部新增微信
  `MrShenzf` 联系方式及复制交互。没有修改云函数、生产数据、云存储、会员订单、
  支付配置或资金状态。
- Verification: 会员与支付页面专项 24/24、全量 Node 709/709、完整
  `npm.cmd run verify`、`git diff --check` 均通过；项目检查为 33 JSON、
  316 JavaScript、13 pages 和 0.60 MiB 客户端源码。微信开发者工具运行态回读
  `role=member`、`purchaseMode=renew`、按钮文本“续费”，并确认底部联系方式可见。
- Code: 业务提交 `d681a44` 已推送至
  `origin/codex/rebuild-digest-inbox`。独立的收藏卡片样式工作树改动未进入提交；
  微信包从该提交的隔离干净工作树生成。
- WeChat: `2.3.4` 于 2026-07-25 19:18:14 上传，包体 1,420,316 bytes，
  项目备注为 `会员续费按钮与咨询联系方式 git-d681a44`。平台送审前回读审核版本
  为空，因此无需执行撤回；上一条 `2.3.3` 审核记录为何不再显示不作推断。
- Review: 送审说明同时覆盖评论互动修复、会员续费按钮和底部微信联系方式；
  沿用已验证的隐私与运行配置。微信公众平台于 2026-07-25 19:47:21 回读
...
## [2026-07-28] direct-global-search-and-recommendation-heat | 一框全站搜索与透明推荐热度

- Session: local Codex task
- Trigger: 用户要求移除搜索页四个范围选择，并希望每条资讯展示 100–1000 的可见数值。
- Search: 页面改为一框直搜；服务端聚合官方动态、资讯、推文、完整 GitHub 库、
  会员专栏和知识简报，使用复合游标按来源分页。普通、Pro、真实管理员的历史和正文
  权益继续由服务端重验；免费用户不会因无简报权益而令整次全站搜索失败。
- Integrity: 没有把随机值伪装成真实喜欢数。列表、详情和资讯类搜索结果显示独立的
  “推荐热度 100–1000”，同一内容稳定不跳变；真实喜欢数、喜欢状态和上游排序分均
  保持原合同。
- Operations: 发布手册改为要求从同一提交部署并回读 `knowledgeFeed` 与
  `knowledgeOps`，并纠正“回填前隐藏入口”的过时说明。回填完成前入口常驻，但实际
  查询明确显示索引准备中。
- Verification: 完整 `npm.cmd run verify` 与 767/767 Node 测试通过；项目检查为
...
## [2026-07-28] comment-ui-review-pause | 待上传替换候选暂时隐藏评论

- Session: local Codex task
- Decision: 通过确定性客户端发布开关将 `comments` 设为 `false`；待上传替换候选不
  显示评论文字、数量、气泡、入口、弹层、会员权益或评论通知，旧评论与资料深链也
  不能恢复评论流程。评论后端、审核、治理和历史数据继续保留。
- Messages: 共享消息会话层过滤现有、历史和未来兼容格式的评论通知，并从仍可见的
  系统消息重新计算未读；消息页不再显示评论分类、互动列表或全局“全部已读”，
  避免隐藏期间误改评论通知的历史已读状态；“我的”通用消息入口从聊天气泡改为
  信封。新版消息请求显式排除评论，服务端沿现有索引分批越过评论通知，避免较早的
  会员/资料通知被最新 50 条评论挤掉；关闭态服务端也拒绝批量已读。
- Verification: 完整 `npm.cmd run verify` 通过，覆盖 38 个 JSON、358 个
  JavaScript、16 个页面、791/791 个 Node 测试、29 个集合合同和 50 个索引合同。
  开发者工具冷启动后，资讯列表、旧评论深链详情、会员、消息、我的和资料编辑六页
  canary 全部通过；两处资讯操作栏均只剩喜欢、收藏、分享且无空槽。
- Release boundary: 当前精确提交、`knowledgeFeed` code-only 部署/回读和微信替换
  版本上传尚待执行；不修改生产集合、索引、数据或存储规则。
  未来恢复评论必须修改中央开关、更新发布测试并提交新的微信审核版本，不得在当前
  审核版本通过后远程热开启。
